...
def build_dataset(configs, dataset, mode='train'):
    ...
    if mode == 'train':
        data_loader = DataLoader(dataset=dataset, batch_size=configs.batch_size, shuffle=True,
                                 collate_fn=bert_batch_preprocessing, num_workers=0)
    else:
    ...
    def __getitem__(self, i):
        docid_list, clause_list, doc_len_list, clause_len_list, pairs, \
            ...
            self.docid_list[i], self.clause_list[i], self.doc_len_list[i], self.clause_len_list[i], self.pairs[i], \
                self._f_emo_query[i], self._f_emo_query_len[i], self._f_emo_query_answer[i], self._f_emo_query_mask[i], \
                self._f_emo_query_seg[i], \
                self._f_cau_query[i], self._f_cau_query_len[i], self._f_cau_query_answer[i], self._f_cau_query_mask[i], \
                self._f_cau_query_seg[i], \
                self._b_cau_query[i], self._b_cau_query_len[i], self._b_cau_query_answer[i], self._b_cau_query_mask[i], \
                self._b_cau_query_seg[i], \
                self._b_emo_query[i], self._b_emo_query_len[i], self._b_emo_query_answer[i], self._b_emo_query_mask[i], \
                self._b_emo_query_seg[i], \
