...
def evaluate_one_batch(configs, batch, model, tokenizer):
    ...
    f_emo_pred = f_emo_pred.cpu().squeeze()  # new added
    ...
    for idx_emo in pred_emo_f:
        ...
        f_query = clause_list[idx_emo] + 'The corresponding causal clauses for this sentence are?'
    ...
    for k in range(len(pred_pair_f)):
        ...
        b_query = clause_list[idx_cau] + 'Which sentence is the emotional clause corresponding to this sentence?'
        ...
        b_adj = bc_adj
        b_emo_pred = model(b_qa, b_mask, b_seg, b_len, b_clause_len, b_doc_len, b_adj)
    ...
    metric_e, metric_c, metric_p = \
        calculate_metrics(pred_emo_final, true_emo, pred_cau_final, true_cau, pred_pair_final, true_pairs, len(clause_list))
...
if __name__ == '__main__':
    configs = Config()
    t = AutoTokenizer.from_pretrained(configs.bert_cache_path)
